...
public class Executor extends Pcl4BaseVisitor<Object> {
    ...
    public Object visitForStatement(Pcl4Parser.ForStatementContext ctx) {
        ...
        Pcl4Parser.ExpressionContext startExprCtx = ctx.expression().get(0);
        Pcl4Parser.ExpressionContext stopExprCtx = ctx.expression().get(1);
        Pcl4Parser.StatementContext loopCtx = ctx.statement();

        String variableName = controlCtx.getText();
        int startValue = (int) Math.round((double) visit(startExprCtx));
        int stopValue = (int) Math.round((double) visit(stopExprCtx));
        int stepValue = (startValue <= stopValue) ? 1 : -1;

        for (int i = startValue; (startValue <= stopValue) ? i <= stopValue : i >= stopValue; i += stepValue) {
            assign(variableName, (double) i);
            visit(loopCtx);
        }
    ...
    public Object visitWriteArgumentList(Pcl4Parser.WriteArgumentListContext ctx) {
        ...
        for (Pcl4Parser.WriteArgumentContext argCtx : ctx.writeArgument()) {
            ...
            if (fwCtx != null) {
                ...
                Pcl4Parser.DecimalPlacesContext dpCtx = fwCtx.decimalPlaces();
                if (dpCtx != null) {
                    format.append(".")
